...
const LineChart = ({ isCustomLineColors = false, isDashboard = false }) => {
  ...
  const datosFetch = async () => {
    const response = await fetch(
      "https://api.thingspeak.com/channels/2263691/fields/1.json?results=13"
    );
      ...
      {dataTemp && (
        <ResponsiveLine
          data={[
            {
              id: "Temperatura",
              color: tokens("dark").greenAccent[500],   //DATATEMP FILTER

              data: dataTemp
              .filter((singleData) => !isNaN(singleData.field1)) // Filtrar valores no numéricos
              .map((singleData) => {
          ...
          margin={{ top: 50, right: 50, bottom: 50, left: 60 }}     // EN EL MARGIN SE REGULA AL QUITAR LAS LEYENDAS
          xScale={{ type: "point" }}
          yScale={{
            type: "linear",
            min: "auto",
