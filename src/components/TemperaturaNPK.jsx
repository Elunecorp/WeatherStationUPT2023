...
const TemperaturaNPK = ({ isCustomLineColors = false, isDashboard = false }) => {
  ...
  const datosFetch = async () => {
    const response = await fetch(
      "https://api.thingspeak.com/channels/2305213/fields/2.json?results=13"
    );
      ...
      {dataTemp && (
        <ResponsiveLine
          data={[
            {
              id: "Temperatura",
              color: tokens("dark").redAccent[500],  // especificamos el color
              ...
              data: dataTemp
              .filter((singleData) => !isNaN(singleData.field2)) // Filtrar valores no numéricos
              .map((singleData) => {
