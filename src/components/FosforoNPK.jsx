...
const FosforoNPK = ({ isCustomLineColors = false, isDashboard = false }) => {
  ...
  const datosFetch = async () => {
    const response = await fetch(
      "https://api.thingspeak.com/channels/2305213/fields/6.json?results=13"
    );
      ...
      {dataTemp && (
        <ResponsiveLine
          data={[
            {
              id: "Temperatura",
              color: tokens("dark").greenAccent[500],  // especificamos el color
              ...
              data: dataTemp
              .filter((singleData) => !isNaN(singleData.field6)) // Filtrar valores no numéricos
              .map((singleData) => {
